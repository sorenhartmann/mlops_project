...
if args.serialize_model:
    ss = 'torch-model-archiver --model-name ' + str(
        args.model_name) + ' --version ' + str(
            args.version) + ' --serialized-file '
    ss = ss + 'torch_serve/' + str(
        args.model_name
    ) + '.pt' + ' --export-path model_store --extra-files torch_serve/index_to_name.json --handler text_classifier'
    os.system(ss)

if args.deploy_model:
    ss = 'torchserve --start --ncs --model-store model_store --models ' + str(
        args.model_name) + '=' + str(args.model_name) + '.mar'
    os.system(ss)
